...
- **`PlaceholderPanel`**: slot accessibility badges, copy-name buttons, and slot content — a text label, an image, or **another skeleton** (parsed via `SpineParser` under its own Cache key, like a compare project; own animation/loop/skin; first named skin auto-picked since symbol skeletons keep art out of `default`). Content sits in `anchor → adjust (X/Y/Scale, bone-local) → content`. The anchor lives in pixi-ext `getSlotContainer` (4.2: draw order + bone transform); on the 4.1 runtime (its slot containers hide while the slot is empty) it's a spine child whose transform is copied from `bone.matrix` each frame. One shared rAF loop drives markers, bone-driven anchors and child-spine **Sync** (mirrors the parent's pause + timeScale).
